...
const createReview = async (req, res) => {
    try {
        ...
        const imagePaths = req.files && req.files.images 
            ? req.files.images.map(f => "/assets/images/reviews/" + f.filename) 
            : [];
        const videoPaths = req.files && req.files.videos 
            ? req.files.videos.map(f => "/assets/videos/reviews/" + f.filename) 
            : [];
